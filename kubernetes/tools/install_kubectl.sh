...
args:
  ...
  -d <download-id>       the directory to download the kubectl binary to [$download_dir]
...
checksum_url="https://dl.k8s.io/release/$version/bin/linux/amd64/kubectl.sha256"
...
if [ -e "$download_dir" ]; then
  ...
  log_info "validating checksum form '$checksum_url'"
  if ! curl --silent --location --output "$download_url/kubectl.sha256"; then
...
if [ "$download_dir" -ne "$install_dir" ]; then
